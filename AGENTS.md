...
8. Customer email: bot and human replies on email conversations are flattened with `markdownToPlainText` and sent with `In-Reply-To`/`References`; the sent mail's RFC id is stored on the message (`markEmailed({ rfcMessageId })`).

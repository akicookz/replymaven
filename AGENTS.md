...
- **Live agent handoff** -- when the bot cannot answer or the visitor requests a human, Maven calls `request_team_help`, then writes a note to the team in the Sidechat. That note goes to every connected Telegram/Slack channel and, by email, to the owner and every accepted member with access. Maven keeps helping while review is pending. Any teammate can write to Maven from any channel; Maven's reply goes back on the channel it came from. A channel or member joins the conversation when a human first replies to the customer there; after that, customer messages go only to the joined routes. The first human reply sets the assignee, and only the assignee can send to the customer through Maven; others are told who has it and offered `assign_conversation`. The inbox Assign menu includes Maven; picking it hands the thread back. Idle takeover after four quiet hours also assigns Maven. New bookings, conversations, and contact form submissions also notify enabled messengers when configured.
...
Sidechat exposes a fixed provider-neutral tool set. `search_knowledge`, `list_knowledge`, `read_knowledge`, `apply_knowledge_change`, and `present_reply_draft` are direct first-party tools. Action tools reuse the dashboard route bodies (`worker/services/conversation-actions.ts`): `reply_to_conversation` posts as Maven in the public thread (chat only), `email_customer` emails the customer (`messageId` for a message already in the chat, or `text` for a new one), `assign_conversation`, `close_conversation`, `block_customer`. `decide_pending_action` is registered only while a tool approval is pending; it records a verified teammate's approve/reject and a continuation turn then runs or denies the call, the same path as the dashboard buttons. Every Sidechat user message carries `origin`, `actorUserId` (tool access principal), `authorUserId` (verified human, empty when unknown), and reply routing (`shared/sidechat-agent.ts` `SidechatUserMeta`). FAQ writes change one pair at its array index. Knowledge writes pause on a Sidechat change card (computed diff plus Approve / Reject). After a decision the card collapses and Maven replies in chat. External MCP and custom HTTP tools use `search_project_tools`, `describe_project_tool`, and `call_project_tool`. Raw external schemas stay inside `MavenProjectAgent`; the model receives bounded summaries and argument guides. A versioned `toolRef` binds each call and approval to the exact connection, tool, catalog fingerprint, safety, and access policy. Manual-write arguments are encrypted in the project Agent until the exact approved call consumes them once; only redacted arguments enter the private transcript.
...
| POST | `/api/widget/:projectSlug/tickets` (alias `/inquiries`) | Ticket form submission; starts a conversation with the form message. Maven answers it like any chat and opens with the configured reply time. No escalation |
...
4. `human_mode` silences the model. The visitor message is forwarded only to the channels where a teammate already replied (`forwardVisitorToJoinedHumans`, through `AgentChannelAdapter.post`). Two exceptions put the model back in play: the visitor addresses `@BotName` directly, and `shouldResumeAiAfterHumanIdle()` — two non-email visitor messages after four quiet hours since the last agent message or human command.
...
8. Escalation is a tool call, not a magic string: the model calls `request_team_help({ customerName?, customerEmail? })` (`worker/chat-runtime/tools/internal/request-team-help.ts`), passing what the customer already said about themselves. `createEscalation` is state only (`waiting_agent`, `escalatedAt`, the `review_summary` pill). The tool then starts a system-origin Sidechat turn (`TEAM_HELP_TRIGGER`); Maven writes the team note and `MavenProjectAgent.mirrorSidechatReply` posts it to every channel. If that turn fails, a fixed fallback line is posted instead. `[HANDOFF_REQUESTED]` survives only in `worker/chat-runtime/streaming/internal-tokens.ts` as a legacy token that gets stripped from output.
...
### Live agent handoff (Telegram, Slack, email)

A channel is a transport for the team thread (`AgentChannelAdapter`, `worker/services/agent-channel.ts`): `resolveConversation` maps an inbound message to a conversation and `post` sends text out. Telegram (`telegram-agent-channel.ts`), Slack (`slack-agent-channel.ts`), and email (`email-agent-channel.ts`) implement it; `listEnabledAgentChannels` builds the set. Nothing above the adapter is channel-specific.

1. The owner saves a Telegram bot token or Slack bot token plus signing secret in Tools. `botName` can be set once, then it is locked. Tokens are encrypted at rest. Telegram uses `ENCRYPTION_KEY` (`worker/services/telegram-secrets.ts`). Slack uses its own derived secrets (`worker/services/slack-secrets.ts`). Email needs only `RESEND_API_KEY`.
2. Telegram: saving registers the webhook with a per-project `secret_token`. The chat id is learned from the first verified update. Slack: Events API at `/api/slack/events/:projectId`. The first trusted `event.channel` binds once. Email: teammate replies arrive on `/api/webhooks/inbound-mail` from a sender matched to the owner or an accepted member.
3. Escalation (`request_team_help`) sets `waiting_agent`, then Maven writes the note in a system-origin Sidechat turn. The mirror posts it to every channel with a new thread: Telegram and Slack thread ids land in `channelThreads`; email sends one mail per recipient (`ProjectService.getEscalationRecipients`) and stores each RFC Message-ID under `channelThreads.email.byUser`, plus the subject (Maven's first line) that every later mail reuses. The RFC id comes from `emails.get` after send; Resend rewrites `Message-ID` and only exposes it there.
4. While review is pending (`waiting_agent`), Maven remains active in `assist_until_agent` and `request_team_help` is hidden.
5. Inbound teammate text goes through `ingestTeammateMessage` (`worker/services/ingest-teammate-message.ts`). If Maven owns the thread, the whole message is a Sidechat turn (`startSidechatTurn` with `origin` = channel; the channel message id is the Sidechat message id, so a redelivered webhook is a no-op). If a human owns it, plain text is a customer-facing reply (`appendHuman`, joins the route, sets the assignee) and a `@BotName` prefix still reaches Maven. A bare `@BotName` assigns Maven.
6. Maven's reply is mirrored to the channel the teammate wrote from, threaded (`replyThreadId` on the Sidechat message: Telegram message id, Slack `thread_ts`, or the teammate's RFC id). Dashboard and MCP turns are not mirrored. The only status-only post is the "working" ack.
7. Author identity: email, dashboard, and MCP turns carry the verified user. Telegram and Slack authors are unknown until channel accounts are linked (phase 6 of the 2026-09-26 plan): Maven talks to them but `reply_to_conversation` and `decide_pending_action` refuse with `unknown_author`.
8. Customer email: Maven's answer to an inbound customer email is emailed back. Dashboard replies on email conversations wait 10 seconds with Undo (`schedulePublicCustomerEmail` / `cancel-email` route). Telegram and Slack human replies only post in the chat; when the customer wrote in by email or is offline and has an email, Maven gets a Sidechat turn whose trigger carries the reply's messageId, and offers to email it (or emails it if told earlier in that Sidechat). Sidechat `email_customer` has the same checks as `reply_to_conversation` and refuses a message already emailed or scheduled. MCP replies are not emailed. Every customer and teammate mail is built by `composeEmail` (`worker/services/email-service.ts`): "Hi {first name}," unless the first line already names them, the body, and a sign-off (author first name plus project for customers, the bot name for teammates); it has a plain-text part and a minimal unstyled HTML part so links show as anchor text. Customer mail comes from "{author} from {project}", and a chat conversation keeps the subject "Your conversation with {project}". Maven is told not to write its own greeting or sign-off. Mail is flattened with `markdownToPlainText`, threaded with `In-Reply-To`/`References`, and the sent mail's RFC id is stored on the message. Emailed messages cannot be deleted.
9. Taking over: assigning a person (dashboard Assign or Maven's `assign_conversation`) makes the thread human-owned; from a channel it also joins that channel's route. After that, plain channel replies go to the customer as that person and `@BotName` reaches Maven.
10. A pending approval does not block new Sidechat turns; the teammate answers with `@BotName approve|reject`. If Maven cannot write the team note (busy or failed), `requestTeamNote` posts a factual fallback. Telegram posts are indexed per message (`channel_message_index` in the project agent) so replies to any bot message resolve. Both webhooks ignore messages from chats/channels other than the connected one. The Telegram link page shows which account it is for and links only on confirm.

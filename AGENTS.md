...
8. Customer email: every reply on an email conversation (bot, dashboard, channel, MCP) is emailed (`deliverMessageToCustomerChannel`), flattened with `markdownToPlainText`, with `In-Reply-To`/`References`; the sent mail's RFC id is stored on the message. Dashboard replies wait 10 seconds with Undo (`schedulePublicCustomerEmail` / `cancel-email` route); channel replies send at once and the channel gets "Emailed to …". Emailed messages cannot be deleted.

...
export async function prepareKnowledgeChange(
  ...
  if (action === "create_faq") {
    ...
    const after = clipText(formatFaqMarkdown(title, pairs, description), MAX_DIFF_CHARS);
    return {
      status: "ready",
      preview: {
        action,
  ...
  if (action === "update_faq") {
    ...
    const title = optionalText(input.title, 200) ?? resource.title;
    const description = optionalText(input.description, FAQ_DESCRIPTION_MAX_CHARS) ??
    ...
    return {
      ...
      preview: {
        ...
        before: clipText(
          formatFaqMarkdown(resource.title, currentPairs, resource.description),
          MAX_DIFF_CHARS,
        ),
        after: clipText(formatFaqMarkdown(title, pairs, description), MAX_DIFF_CHARS),
        reason,
      },
      write: {
        action,
...
export function isKnowledgeChangeActionName(
  value: unknown,

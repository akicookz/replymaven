...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      "Connect your APIs and workflows for account lookups, upgrades, refunds, internal escalation, and issue creation. You control each tool and when Maven can use it.",
  },
  {
...
const MCP_CLIENTS = [
  ...
  },
  {
    ...
    instructions:
      "Settings → Connectors → Create connector, then paste the URL.",
  },
  {
    name: "Cursor",
    logo: "/integrations/cursor.svg",
    instructions:
      "Add the server URL to ~/.cursor/mcp.json under mcpServers.",
  },
  {
    name: "Conductor",
    logo: "/integrations/conductor.svg",
    instructions:
      "Add ReplyMaven to your agent's MCP config, then refresh MCP status.",
  },
] as const;
        ...
        {MCP_CLIENTS.map((client) => (
          <div key={client.name}>
            <div className="flex items-center gap-3">
              <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-white/[0.045] shadow-[0_0_0_1px_rgba(255,255,255,0.07)]">
                <img
                  src={client.logo}
                  alt=""
                  width={24}
            ...
            <p className="mt-4 text-pretty text-xs leading-5 text-ink-6">
              {client.instructions}
            </p>
          </div>
        ))}
...
const AGENT_TRACE = [
  {
    icon: Search,
    title: "Looked up the subscription in Stripe",
    detail: "Pro plan · $49 per month",
  },
  {
    icon: Zap,
    title: "Calculated the upgrade charge",
    detail: "$41 mid-cycle proration",
  },
  {
    icon: FileText,
    title: "Checked the billing policy",
    detail: "1 source cited",
  },
] as const;
    ...
    <Window className="flex min-h-[460px] items-center bg-[#0b0c10] p-5 sm:p-9">
      <div className="w-full max-w-[580px] mx-auto">
        ...
        <div className="flex flex-col items-end">
          ...
          <div className="mb-2 ml-auto w-full max-w-[92%] px-1 py-2.5 sm:max-w-[76%]">
            <p className="pr-7 text-right text-[10.5px] font-normal text-ink-7">
              Used 2 tools and 1 source
            </p>
            <div className="relative mt-2.5">
              <span
                aria-hidden
                className="absolute bottom-4 right-[9px] top-4 w-px bg-white/[0.06]"
              />
              {AGENT_TRACE.map((step) => {
                const StepIcon = step.icon;

                return (
                  <div
                    key={step.title}
                    className="relative flex flex-row-reverse items-start gap-2.5 py-1.5"
                  >
                    <span
                      className="relative z-10 flex size-5 shrink-0 items-center justify-center rounded-full bg-[#0b0c10] text-ink-7"
                    >
                      <StepIcon className="size-3" />
                    </span>
                    <div className="min-w-0 text-right">
                      <p
                        className="text-pretty text-[11px] font-normal text-ink-6"
                      >
                        {step.title}
                      </p>
                      <p className="mt-0.5 text-[10px] text-ink-8">
                        {step.detail}
                      </p>
                    </div>
                  </div>
...
function FaqItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div>
      ...
      <div className={cn("grid transition-[grid-template-rows,opacity] duration-200 ease-in-out motion-reduce:transition-none", open ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0")}>
